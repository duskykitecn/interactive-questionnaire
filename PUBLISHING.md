...
**日常**：主库 `git push` 之后最多等一个 crontab 周期；发版打了 `v*` 标签也会被 `push_tags` 拉过去。GitHub Release 附件不会出现在 CNB，下载继续指 README 里的 GitHub Releases。
...
## 演示站（多项目共用 pages 子域，不走 COS）

任意项目的静态演示页都挂在同一对 **pages 子域** 的 `/项目名/` 下（不限于技能）。本项目三处地址：
...
命名统一用 **pages**：子域名是 `pages`，总仓是 `duskykitecn/pages`。根域 `duskykite.com.cn` / `duskykite.xyz` 留给以后的官网，不跟演示页抢路径。

**不要用 COS。** 演示页是静态 HTML，用免费托管即可。

**也不要把每个项目仓库单独接成 EdgeOne Pages。** 一个 Git 项目占整个主机名的 `/`，项目之间会抢。正确做法是单独一个 **pages 总仓**，根目录按项目名分文件夹；`pages.*` 两个主机名都发布这个总仓。

```
duskykitecn/pages          ← 总仓，只放拍扁后的静态页
├── .nojekyll
...
不要在总仓放 GitHub Pages 的 `CNAME`：两个 `pages.` 都走 EdgeOne，再写 `CNAME` 会把 xyz 抢回 GitHub。
...
各项目仓库 push main
    ...
    └─ Actions 只更新总仓里的 /<项目名>/
            ├─ EdgeOne 项目 A（中国大陆）
            │    pages.duskykite.com.cn/<项目名>/
            └─ EdgeOne 项目 B（全球不含大陆）
                 pages.duskykite.xyz/<项目名>/
```

原理：DNS 把 `pages.duskykite.*` 指到 **EdgeOne**，不是 GitHub。加速区域是项目级，所以国内、海外各建一个 Makers 项目，都导入同一个 `duskykitecn/pages`。文件夹名仍是 URL 路径。

免费：Makers 免费版约 40 个项目、200 个自定义域名、免费证书；静态 HTML 几乎用不完。免费版不承诺 SLA，日后商业化可能收紧配额。`.com.cn` 已备案所以能走国内节点；`.xyz` 选「不含大陆」故不必备案，访问也不经过 GitHub。

本项目日常：改 `interactive-questionnaire/assets/demo.html` 并 push `main`。本仓库 GitHub Pages 自动更新；打开了总仓同步后，两个 `pages.` 地址一起更新。其它项目复制 `scripts/build_site.py` + `deploy-site.yml`，把目录名改成自己的项目名即可。
...
```bash
gh repo create duskykitecn/pages --public --add-readme --description "DuskyKite project demo pages"
```

总仓根保留 `.nojekyll`。若已误加 GitHub 用的 `CNAME`，删掉并推送（见下）。可选再放一个简单的 `index.html` 作目录。总仓 **不必** 开 GitHub Pages 自定义域名。

2. **EdgeOne 项目 A（国内）**：开通 Makers 免费版 → 导入 `duskykitecn/pages` → 分支 `main`，无构建、输出 `.`。加速区域 **中国大陆可用区**。绑定 `pages.duskykite.com.cn`，免费证书。DNS（com.cn）：CNAME 主机记录 `pages` → 该项目控制台给出的地址。

3. **EdgeOne 项目 B（海外）**：再新建一个 Makers 项目，导入 **同一个** GitHub 仓。加速区域 **全球可用区（不含中国大陆）**（不必备案）。绑定 `pages.duskykite.xyz`，免费证书。DNS（xyz）：CNAME 主机记录 `pages` → **这个**项目控制台给出的地址（与国内那条不是同一个 CNAME 目标）。两个项目不要交叉绑定域名。

4. **本仓库打开同步**：建一个**只对 `duskykitecn/pages` 有 Contents 写权限**的 fine-grained PAT。在本仓库 Settings → Secrets 加 `PAGES_DEPLOY_TOKEN`（在 GitHub 网页粘贴）。Settings → Variables 加 `PAGES_DEPLOY_ENABLED=true`、`PAGES_REPO=duskykitecn/pages`。Actions 里手动跑一次 **Deploy demo site**。

5. **以后新项目**：同一套脚本，把 `PROJECT_SLUG` / 构建输出目录改成新项目名；PAT 可复用。总仓和两个 EdgeOne 项目都不用再建。

常见坑：把某个项目的整仓原样挂到 `/项目名/` 会双重路径。必须用 `build_site.py` 的拍扁产物。workflow 只 `rm -rf` 再拷入本项目那一层。
...
7. 若改过 `demo.html`：push 到 `main` 后本仓库 GitHub Pages 自动更新；打开了 `PAGES_DEPLOY_ENABLED` 则 `pages.` 子域随总仓一起更新（见上文「演示站」）。
...
- About：Description 与 Topics（如 `agent-skills`）；Website 可先填 GitHub Pages，`pages.` 子域通了再改。
...
- 不必每次上传发行版附件，README 已指向 GitHub Releases。

**CNB 镜像**

- 仓在组织下；首次 `--mirror` 灌入后走 `.cnb.yml` git-sync。
- 环境变量只放在 CNB 控制台；简介同样标明镜像。
- 关掉镜像上的协作入口。
...
- **演示站双重路径**：`pages.` 主机名必须用 `scripts/build_site.py` 的拍扁产物挂到总仓 `/项目名/`，不要把整仓原样上传，也不要为每个项目单独接 EdgeOne Pages。
- **演示站不要用 COS**：静态页走 GitHub Pages + EdgeOne Makers 免费版即可。
